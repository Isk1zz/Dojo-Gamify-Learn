...
// Chunk XP is `15 + Math.floor(Math.random() * 7)` in library.js — a
// range, not a number, and the server rolls it. Read the two ends the
// same way so a change to the jitter follows automatically.
function readChunkXp() {
  const src = fs.readFileSync(LIBRARY, "utf8");
  const m = src.match(/const\s+gain\s*=\s*(\d+)\s*\+\s*Math\.floor\(Math\.random\(\)\s*\*\s*(\d+)\)/);
  if (!m) throw new Error("chunk XP expression not found in library.js");
  const base = Number(m[1]), span = Number(m[2]);
  return { min: base, max: base + span - 1 };
}
...
function main() {
  ...
  console.log("insert into public.content_items (id, kind, xp_min, xp_max, money, tokens, min_seconds) values");
  console.log(rows.map(r =>
    `  (${q(r.id)}, ${q(r.kind)}, ${r.xp_min}, ${r.xp_max}, ${r.money}, ${r.tokens}, ${r.min_seconds})`
  ).join(",\n"));
