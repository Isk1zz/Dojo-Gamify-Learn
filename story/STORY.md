...
and **is** a story day now. `resolveChoice` calls `LifeShop.storyDayTick()`
right after that vitals cost, which rolls the nightly theft check. There is
no separate real-calendar-day tick any more; see `shop/SHOP.md`.
