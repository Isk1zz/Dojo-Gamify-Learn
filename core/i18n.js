...
const I18N = (() => {
  ...
  const STRINGS = {
    ...
    "legal.privacyLocal":   { en: "<strong>On this device.</strong> Your profile, progress, review schedule, statistics and settings are kept in this browser's local storage. That is what lets you keep studying without a network on a device you have already signed in on.",
                              ru: "<strong>На этом устройстве.</strong> Профиль, прогресс, расписание повторений, статистика и настройки хранятся в локальном хранилище браузера. Благодаря этому можно продолжать учиться без сети — на устройстве, где вы уже входили." },
    ...
    "legal.privacyForum":   { en: "<strong>What other people can see.</strong> If you post on the Forum, that post and its replies are visible to everyone signed in, together with your nickname, avatar and any badges you pinned. Nothing else about you is shown to them — not your email, not your country, not your progress or settings. Reputation you give is recorded, so an author can see their own total, but who gave it is not displayed.",
                              ru: "<strong>Что видят другие.</strong> Если вы пишете на форуме, запись и ответы к ней видны всем вошедшим — вместе с ником, аватаром и закреплёнными значками. Больше о вас им не показывают ничего: ни почту, ни страну, ни прогресс, ни настройки. Отданное признание записывается, поэтому автор видит свой итог, но кто именно дал — не отображается." },
    "legal.privacyNoTrack": { en: "There is no analytics, no telemetry, no advertising and no third-party tracking of any kind. Nothing about you is sold or handed to anyone else, and apart from what you deliberately post on the Forum, your data is used only to show it back to you.",
                              ru: "Нет аналитики, телеметрии, рекламы и никакого стороннего отслеживания. Ничего о вас не продаётся и не передаётся третьим лицам, и кроме того, что вы сами публикуете на форуме, ваши данные используются только чтобы показать их вам." },
...
// ---- Interface pass 4: section titles and the prose under them ----
    ...
    "forum.viewsTip":       { en: "People who actually read this - scrolling past does not count",
                              ru: "Сколько людей действительно прочли — пролистывание не считается" },
